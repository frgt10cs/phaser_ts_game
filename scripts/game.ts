...
import { DamageRune } from "./damageRune";
import { HealingRune } from "./healingRune";
...
export class GameScene extends Phaser.Scene {
  ...
  private enemySpawnedCount: number;
  ...
  private readonly maxRunesCount: number;
  private readonly damageItemDamage: number;
  private readonly firstAidHeal: number;
  ...
  constructor() {
    ...
    this.maxRunesCount = 2;
    this.damageItemDamage = 2;
    this.firstAidHeal = 4;
  }
  ...
  public create() {
    ...
    this.input.keyboard.on('keydown_M', this.chanegMusicState, this);
    ...
    this.physics.add.overlap(this.player.sprite, firstAids,
      (playerSprite, aid: Phaser.Physics.Arcade.Sprite) => {
        this.player.playSound("heal");
        this.player.heal(this.firstAidHeal);
        aid.destroy();
      }, null, this);
    ...
    this.physics.add.overlap(this.player.sprite, damageItems,
      (playerSprite, damageItem: Phaser.Physics.Arcade.Sprite) => {
        this.player.hurt(this.damageItemDamage);
        damageItem.destroy();
      }, null, this);

    // runes
    let ddEmitterManager = this.add.particles("ddParticle");
    let regenEmitterManager = this.add.particles("regenParticle");    
    ...
    let runeSprite: Phaser.Physics.Arcade.Sprite;
    let runeInterval = setInterval(() => {
      if (this.scene.isPaused()) return;
      if (this.currentRune != null) {
        this.currentRune.sprite.destroy();
      }
      let randomInt = Math.floor(Math.random() * this.maxRunesCount);
      switch (randomInt) {
        case 0:
          runeSprite = this.physics.add.sprite(runeSpawner.x, runeSpawner.y - 20, "damageRune");
          this.currentRune = new DamageRune("doubleDamage", runeSprite, ddEmitterManager);
          break;
        case 1:
          runeSprite = this.physics.add.sprite(runeSpawner.x, runeSpawner.y - 20, "healingRune");
          this.currentRune = new HealingRune("regeneration", runeSprite, regenEmitterManager);
          break;
      }
  ...
  private createEnemy(x: number, y: number): void {
    ...
    this.enemySpawnedCount++;
    if (this.enemySpawnedCount % 5 == 0) {
      this.waveNumber++;
    }
